...
class SupportVisit(TransactionBase):
			...
			for d in self.get("purposes"):
				if d.prevdoc_docname and d.prevdoc_doctype == "Warranty Claim":
					if flag == 1:
						...
						nm = frappe.db.sql(
							"select t1.name, t1.mntc_date, t2.service_person, t2.work_done from `tabSupport Visit` t1, `tabSupport Visit Purpose` t2 where t2.parent = t1.name and t1.completion_status = 'Partially Completed' and t2.prevdoc_docname = %s and t1.name!=%s and t1.docstatus = 1 order by t1.name desc limit 1",
	...
	def check_if_last_visit(self):
		...
		if check_for_docname:
			check = frappe.db.sql(
				"select t1.name from `tabSupport Visit` t1, `tabSupport Visit Purpose` t2 where t2.parent = t1.name and t1.name!=%s and t2.prevdoc_docname=%s and t1.docstatus = 1 and (t1.mntc_date > %s or (t1.mntc_date = %s and t1.mntc_time > %s))",
			...
			if check:
				...
				frappe.throw(
					_("Cancel Support Visits {0} before cancelling this Support Visit").format(check_lst)
				)
				raise Exception
			else:
